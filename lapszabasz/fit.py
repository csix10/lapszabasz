...
class FitAlgorithms:
  ...
  def NFD(self):
      ...
      for rec in sort_rec:
        ...
        if sum < 1:
          ...
          remnant.append((1-sum+rec[1]) * lane[0][0])
          sum = rec[1]
          lanes.append(lane)
          lane=[rec]
      lanes.append(lane)
  ...
  def BFD(self):
    ...
    for rec in sort_rec:
      ...
      # Az első lista rendezése a megadott feltételek szerint
      sizes = [sizes[i] for i in sorted_indices]
      # A másik lista ugyanabban a sorrendben
  ...
  def BFD_lanes(self):
    ...
    lanes = sorted(lanes, key=lambda x: x[0][0], reverse=1)
    sizes=[]
    tables=[]
        ...
        for lane in table:
          for rec in lane:
            ...
            if rec[0]>0.1 and rec[1]>0.1:
              ...
              dwg.add(dwg.text(round(rec[1],2), insert=text_position_2, text_anchor="middle", transform=f"rotate(270, {text_position_2[0]}, {text_position_2[1]})"))
            y = y + height
          y = 0
          x = x + lane[0][0]*u
        i=i+1
